<form wire:submit.prevent="update_categories">
                ...
                @foreach($all_categories as $category)
                    <tr class="bg-white border-b hover:bg-gray-50 ">
                        <td class="w-4 p-4">
                            ...
                            <input wire:model.defer="categories" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" value="{{ $category->id }}" >
                            </div>
                        </td>
   ...
   <div class="flex flex-row items-center justify-between p-5 border-t border-gray-200">
        ...
        <button type="submit" class="px-4 py-2 text-white font-semibold bg-indigo-500 hover:bg-indigo-700 rounded">{{ __('categorypage.add') }}
        </button>
    </div>
</form>